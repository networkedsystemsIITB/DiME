...
do
	...
	if [ $bench -eq 1 ]; then
		throughput=$(cat $run_log | grep "Totals" | awk '{print $2;}' | tr "\n" '\t');
	else
		throughput=$(cat $run_log | grep "Throughput(ops/sec)" | awk -v FS='[ ,]+' '{print " ", $3;}' | tr "\n" '\t');
	fi

	echo -n $basename | tr '-' ' '
	echo -n " throughput $throughput"
	awk -f ${scriptdir}/extract_run_stats.awk $run_core_stats $load_core_stats;
	awk -f ${scriptdir}/extract_run_stats.awk $run_prp_stats $load_prp_stats;
	echo ""
